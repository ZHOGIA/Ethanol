...
local function createNormalEmbed(fishName, playerName, tierName, tierColor, tierEmoji, weight, mutationStatus, fishPrice)
    return {
        ...
        footer = {
            text = "Ethanol • " .. os.date("%m/%d/%Y %I:%M %p")
        },
        timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
    }
end
        ...
        embeds = {
            {
                ...
                description = "Ethanol webhook system is now connected and ready!",
                color = 0x00FF00,
                fields = {
                    {
                        name = "**Status**",
...
-------- MENDAFTAR SEMUA FITUR KEDALAM CONFIG 
...
local SecuritySection = Security:Section({ 
    Title = "Humanization Settings",
})

-- Fungsi untuk delay acak
local function randomWait(min, max)
    local waitTime = math.random(min * 1000, max * 1000) / 1000
    task.wait(waitTime)
end

-- Toggle untuk mengaktifkan humanisasi
local humanizeActions = false

local HumanizeToggle = Security:Toggle({
    Title = "Enable Action Humanization",
    Desc = "Adds random delays to actions to look more human",
    Default = true, -- Selalu aktifkan ini
    Callback = function(state)
        humanizeActions = state
    end
})

-- Slider untuk mengatur tingkat humanisasi
local humanizationLevel = 50 -- Default 50%

local HumanizationSlider = Security:Slider({
    Title = "Humanization Level (%)",
    Desc = "Higher = more random delays",
    Step = 10,
    Value = {Min = 10, Max = 100, Default = 50},
    Callback = function(value)
        humanizationLevel = value
    end
})
...
local function simulateHumanActivity()
    local VirtualInputManager = game:GetService("VirtualInputManager")
    
    -- Gerakan mouse acak
    local randomX = math.random(100, 500)
    local randomY = math.random(100, 500)
    VirtualInputManager:SendMouseMoveEvent(randomX, randomY, nil)
    
    -- Kombinasi tombol acak
    local keyCombos = {
        {Enum.KeyCode.LeftShift, Enum.KeyCode.E},    
        {Enum.KeyCode.LeftControl, Enum.KeyCode.F},     
        {Enum.KeyCode.Q, Enum.KeyCode.Tab},           
...
local AntiAFKToggle = Security:Toggle({
    Title = "Enhanced Anti-AFK",
    Desc = "Prevents kick with mouse movement and key presses",
    Default = true,
    Callback = function(state)
        ...
        if state then
            antiAFKLoop = task.spawn(function()
                while antiAFKEnabled do
                    -- Interval acak antara 5-15 menit
                    local waitTime = math.random(300, 900)
                    for i = 1, waitTime do
...
local safetyDistance = 100 -- Default 100 studs
local safetyLoop = nil

local SafetyToggle = Security:Toggle({
    Title = "Player Near Safety",
    Desc = "Disables features when other players are nearby",
    Default = false,
    Callback = function(state)
        safetyEnabled = state
                        ...
                        for _, player in pairs(game.Players:GetPlayers()) do
                            if player ~= localPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                                ...
                                if distance < safetyDistance then
                                    -- Nonaktifkan semua fitur utama
                                    if autoFishingRunning then
                                        autoFishingToggle:Set(false) -- Ini akan memicu callback untuk mematikan auto fishing
                                        WindUI:Notify({
                                            Title = "Safety Alert",
                                            Content = "Player nearby! Features disabled.",
                                            Duration = 5,
                                            Icon = "alert-triangle"
                                        })
                                    end
                                    
                                    -- Tunggu sampai pemain pergi
                                    while distance < safetyDistance do
                                        task.wait(2)
...
local SafetySlider = Security:Slider({
    Title = "Safety Distance (Studs)",
    Step = 25,
